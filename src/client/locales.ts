/**
 * This plugin's copy, in the two locales the shell ships.
 *
 * Registered through `ctx.locale.register`, so the text follows the user's own
 * language choice rather than a build-time decision, and switching the language
 * in Settings re-renders these strings with everything else. The lookup chain
 * falls back to `en` per key, which is why both dictionaries carry the same key
 * set: a key present in one and missing from the other would silently show
 * English inside an otherwise Chinese page.
 */

export const LOCALE_NS = 'dsh-ext'

/** The key domain. One object literal so a missing translation is a type error. */
export const en = {
  // Section titles and descriptions
  'section.images': 'Composer images',
  'section.images.desc': 'An image entry at the top of the + menu, and drag-to-reorder for draft images.',

  // Settings tabs
  'tab.input': 'Input & models',
  'tab.balance': 'Balance',
  'tab.review': 'Command Review',
  'tab.files': 'Files',
  'tab.sessions': 'Sessions',
  'tab.plugins': 'Fault Rescue',

  // Balance extras
  'balance.peak': 'Peak',
  'balance.offPeak': 'Off-peak',
  'balance.poll': 'Poll interval (seconds)',
  'balance.poll.hint': 'The balance chip refreshes itself every N seconds. 0 disables polling.',
  'balance.peakWindows': 'Peak windows (Beijing time)',
  'balance.peakWindows.hint': 'Official defaults converted to Beijing time: weekdays 09:00-12:00 and 14:00-18:00; rates are half outside these windows. Comma-separated HH:MM-HH:MM.',
  'balance.peakWeekdays': 'Weekends are off-peak',
  'balance.peakWeekdays.hint': 'Official scheme: Saturdays and Sundays bill at off-peak rates all day.',
  'balance.badge.title': 'DeepSeek balance · peak windows {windows} (Beijing time) · key from {source}',
  'section.effort': 'Reasoning effort',
  'section.effort.desc': 'Declares each model\'s effort levels for third-party (pi-ai) routes, so the composer\'s effort control appears for those models.',
  'section.balance': 'DeepSeek balance',
  'section.balance.desc': 'Reads the account balance of the configured DeepSeek official API key. The key never reaches the browser.',
  'section.review': 'Command review',
  'section.review.desc': 'Screens high-risk tool calls with local patterns, then asks a second model to judge the ones that match. A review can only make a call stricter.',
  'section.explorer': 'Project explorer',
  'section.explorer.desc': 'The workspace directory tree and its uncommitted changes. Read-only: every git command behind it is a query.',
  'section.sessions': 'Sessions',
  'section.sessions.desc': 'The recycle bin is in the sidebar foot, beside Settings. Turning session administration off hides the bin and lets undo/edit leave the original session in place.',
  'section.plugins': 'Fault Rescue',
  'section.plugins.desc': 'Manage plugin quarantines and standalone rescue commands to recover from startup failures.',
  'section.checkpoints': 'Checkpoints',
  'section.checkpoints.desc': 'Per-session rollback through a shadow git repository. Your project\'s own git history, index, and stashes are never touched.',

  // Shared controls
  'common.enabled': 'Enabled',
  'common.on': 'On',
  'common.off': 'Off',
  'common.refresh': 'Refresh',
  'common.clear': 'Clear',
  'common.close': 'Close',
  'common.cancel': 'Cancel',
  'common.save': 'Save',
  'common.reset': 'Reset',
  'common.delete': 'Delete',
  'common.restore': 'Restore',
  'common.yes': 'Yes',
  'common.no': 'No',
  'common.loading': 'Loading…',
  'common.days': 'days',
  'common.readonly': 'No settings provider is mounted in this deployment, so changes cannot be stored. The values below are the ones the composition supplied.',

  // Images
  'images.button': 'Image entry in the + menu',
  'images.button.hint': 'Adds "Add images" as the first entry of the + menu.',
  'images.drag': 'Drag to reorder',
  'images.drag.hint': 'Drag a thumbnail, or use its ‹ › buttons. Reordering is refused while a message is being sent.',
  'images.add': 'Add images',
  'images.pickHint': 'Attach images to this message',
  'files.attach': 'Attach a file',
  'files.attachHint': 'Pick a file from this computer and attach it',
  'files.notImage': '{name} is not an image, so its text was added to the message instead.',
  'files.unreadable': '{name} could not be attached: it is neither an image nor text.',
  'images.remove': 'Remove {name}',
  'images.earlier': 'Move {name} earlier',
  'images.later': 'Move {name} later',
  'images.busy': 'The composer is busy sending; the image order was left as it was.',
  'images.drop': 'Drop images to attach them',
  'images.dropLimits': 'Drop up to {count} image(s), {size} each',
  'images.rail': 'Draft images',
  'images.railMissing': 'The image rail is not mounted',

  // Effort
  'effort.models': 'Models and levels',
  'effort.defaultFull': 'Full efforts by default',
  'effort.defaultFull.hint': 'Models without an explicit override receive off, minimal, low, medium, high, xhigh, and max.',
  'effort.none': 'No third-party (pi-ai) providers are configured. Add one on the Models page first.',
  'effort.noEfforts': 'no efforts',
  'effort.fromAdapter': 'from the adapter: {list}',
  'effort.declare': 'Declare',
  'effort.edit': 'Edit',
  'effort.notLoaded': 'configured, not loaded',
  'effort.level': 'Level',
  'effort.sentAs': 'Sent as',
  'effort.explain': 'The left column is what you pick in the composer. The right column is what the provider is sent — leave it empty only for "off", which sends nothing.',
  'effort.add': 'Add:',
  'effort.remove': 'Remove',
  'effort.noReasoning': 'No reasoning',
  'effort.noReasoning.hint': 'Record that this model does no reasoning at all',
  'effort.reset.hint': 'Remove this plugin\'s override and inherit whatever the adapter says',
  'effort.sendNothing': '(send nothing)',
  'effort.required': 'required',
  'effort.readonly': 'No settings provider is mounted, so efforts cannot be stored.',

  // Vision (the image-modality declaration)
  'vision.on': 'Images ✓',
  'vision.off': 'Images ✕',
  'vision.defaultAll': 'Images on by default',
  'vision.defaultAll.hint': 'Models without an explicit modality declaration are configured for text and image input.',
  'vision.hint': 'Whether this model accepts image attachments. The host refuses images unless the model declares the image modality.',

  // Composer model menu (this plugin's replacement for the shipped one)
  'section.modelPicker': 'Model menu',
  'section.modelPicker.desc': 'The composer\'s model list, with each provider group collapsible and a filter by name.',
  'modelPicker.collapse': 'Collapsible groups',
  'modelPicker.collapse.hint': 'Off shows every provider expanded at all times, as the shipped menu did.',
  'picker.chooseModel': 'Select model',
  'picker.triggerAria': 'Select model, currently {model}',
  'picker.triggerAriaEffort': 'Select model, currently {model}, reasoning {effort}',
  'picker.menuAria': 'Model and reasoning level',
  'picker.model': 'Model',
  'picker.effort': 'Reasoning',
  'picker.providerDefault': 'Default',
  'picker.loading': 'Refreshing the model list…',
  'picker.actionFailed': 'The model operation failed: {message}',
  'picker.reload': 'Reload',
  'picker.groupFailed': '{name} failed to load: {message}',
  'picker.noModels': 'No models are available.',
  'picker.noMatch': 'No model matches that.',
  'picker.noEfforts': 'This model exposes no reasoning levels.',
  'picker.filter': 'Filter models',
  'picker.collapseAll': 'Collapse all',
  'picker.expandAll': 'Expand all',

  // Composer effort control
  'effortPicker.label': 'Reasoning effort',
  'effortPicker.none': 'This model exposes no reasoning levels.',
  'effortPicker.default': 'Provider default',
  'effortPicker.declaring': 'Declaring…',
  'effortPicker.declareStandard': 'Enable standard levels (off/low/medium/high)',
  'effortPicker.notDescribed': 'The installed catalog does not describe this route, so levels cannot be declared for it here. Spell the model out in the provider’s model list on the Models page first.',
  'effortPicker.failed': 'The model refused that effort level.',

  // Balance
  'balance.currency': 'Currency',
  'balance.total': 'Total',
  'balance.granted': 'Granted',
  'balance.toppedUp': 'Topped up',
  'balance.badge': 'Composer chip',
  'balance.badge.hint': 'Also show a compact balance chip in the composer, just left of the model selector.',
  'balance.reading': 'Reading the balance…',
  'balance.unavailable': 'DeepSeek reports this account cannot currently serve requests.',
  'balance.noRows': 'The API reported no balance rows.',
  'balance.keyFrom': 'key from {source}',

  // Review
  'review.mode': 'Mode',
  'review.mode.rules': 'Local rules only',
  'review.mode.rulesLlm': 'Rules, then model',
  'review.mode.all': 'Every covered call',
  'review.provider': 'Reviewer provider',
  'review.provider.hint': 'The provider route the reviewer runs on. Any route configured on the Models page.',
  'review.model': 'Reviewer model',
  'review.model.hint': 'The model id the reviewer asks. Press Enter or click away to save.',
  'review.timeout': 'Review timeout (ms)',
  'review.timeout.hint': 'How long to wait for a verdict before falling back to the rule below (1000–120000 ms).',
  'review.deadline': '{ms} ms deadline',
  'review.onFailure': 'On reviewer failure',
  'review.onFailure.hint': 'What happens when the reviewer times out, errors, or has no credential.',
  'review.onFailure.ask': 'Ask the user',
  'review.onFailure.deny': 'Refuse the call',
  'review.onFailure.allow': 'Allow and log',
  'review.tools': 'Reviewed tools',
  'review.tools.hint': 'Tool names subject to review; edit the list in settings.yaml.',
  'review.writeOnly': 'Review writes only',
  'review.writeOnly.hint': 'Skip recognized literal read commands. Full review mode does not skip reads; concurrency metadata grants no exemption.',
  'review.denyPatterns': 'High-risk rules',
  'review.denyPatterns.hint': 'One regular expression per line. Matching or unclassified calls are escalated or sent to the model.',
  'review.readPatterns': 'Read-only command rules',
  'review.readPatterns.hint': 'One whole-command regular expression per line. These narrow the built-in literal read classifier; execution options cannot be exempted.',
  'review.absoluteDelete': 'Absolutely prohibit deletion',
  'review.absoluteDelete.hint': 'Recognized deletion operations are denied immediately. The model and human approval cannot override this.',
  'review.deleteCommand': 'Deletion commands',
  'review.gitPushCommand': 'git push commands',
  'review.commandPolicy.deny': 'Absolutely prohibit',
  'review.commandPolicy.ask': 'Ask the user',
  'review.commandPolicy.allow': 'Allow',
  'review.commandPolicy.expected': 'Review against intent (default)',
  'review.mode.default': 'Default',
  'review.mode.expected.label': 'Review against intent',
  'review.mode.expected.hint': 'For other commands, compare source-verified human requests with actual arguments, cwd and target observations. Check quoting, whitespace, variables and unexpectedly broad effects. Confirm uncertainty; reject evidenced serious mistakes. Recognized reads may skip review when writes-only is enabled. Deletion and push use their independent policies.',
  'review.mode.rules+llm.label': 'Rules + model review',
  'review.mode.rules+llm.hint': 'For other commands, screen locally and send risky or unclassified operations to AI for general safety review, without reconstructing task intent. Recognized reads may skip AI. Deletion and push are handled exclusively by their own policies.',
  'review.mode.rules-only.label': 'Local rules only',
  'review.mode.rules-only.hint': 'For other commands, use local rules without AI; recognize reads and ask about uncertain writes. A deletion or push policy set to intent review still uses AI independently of this mode.',
  'review.mode.all.label': 'Review all with AI',
  'review.mode.all.hint': 'Send every covered other operation, including reads, to AI for general safety review. This increases requests and latency, not automatic refusals. Deletion and push remain governed only by their independent policies.',
  'review.deletePolicy.deny.hint': 'Refuse recognized deletion operations immediately. Neither AI nor a confirmation can override this choice, even when deletion fits the task. Other operations in a compound command do not weaken this refusal.',
  'review.deletePolicy.ask.hint': 'Require your confirmation for each recognized deletion operation. AI cannot approve it and the global review mode does not apply to this category. Other operations in the same command are checked separately; host restrictions still apply.',
  'review.deletePolicy.expected.hint': 'Independently compare deletion targets with the human task, checking argument splitting, variables, wildcards and project-root mistakes. Allow evidenced, correctly scoped work; ask about missing evidence or critical/unresolved targets; reject clear serious mistakes. This runs even in local-rules mode.',
  'review.deletePolicy.allow.hint': 'Permit recognized deletion operations without this plugin\'s AI review or user confirmation, regardless of global review mode. Other operations in a compound command are still checked separately. Host permissions remain in force; syntax that cannot be safely separated requires confirmation.',
  'review.gitPushPolicy.deny.hint': 'Refuse recognized Agent git push operations immediately, including normal, force and tag pushes. Neither AI nor user confirmation overrides this choice. This does not change the manually operated Git push button.',
  'review.gitPushPolicy.ask.hint': 'Require your confirmation for each recognized Agent push. Completed edits or passing tests do not authorize automatic pushing. The global review mode does not apply to this category; other operations and host restrictions are still checked.',
  'review.gitPushPolicy.expected.hint': 'Independently verify that the human requested a push and that remote, refspecs and flags match. Editing code or committing locally is not push authorization. Ask when intent or implicit targets are unresolved; reject evidenced serious target mistakes. This runs regardless of global mode.',
  'review.gitPushPolicy.allow.hint': 'Permit recognized Agent pushes without this plugin\'s AI review or user confirmation, regardless of global review mode. This allows automatic pushing. Other operations in a compound command and host permissions remain independently enforced; inseparable syntax requires confirmation.',
  'review.deletePatterns': 'Deletion rules',
  'review.deletePatterns.hint': 'One regular expression per line, matched against tool identity and executable syntax, not quoted search text or patch additions.',
  'review.verdicts': 'Recent verdicts',
  'review.empty': 'No reviewed commands yet.',
  'review.count': '{n} most recent verdict(s).',
  'review.matched': 'matched pattern:',
  'review.off': 'Command review is switched off.',
  'review.modelPick': 'Reviewer model',
  'review.modelPick.hint': 'Choose a fixed reviewer or follow the calling session model. Safety instructions stay independent; a missing session model uses the review failure policy.',
  'review.model.followSession': 'Follow current session model (Default)',
  'review.autoChip': 'Auto review',
  'review.autoChip.hint': 'Every write command is adjudicated by the reviewer model first; commands whose risk the model cannot judge escalate to you.',
  'review.autoReview': 'Chat auto-review',
  'review.autoReview.hint': 'When enabled, commands in chat sessions are screened and reviewed. When disabled, review is paused.',
  'review.section.model': 'Review mode & model strategy',
  'review.section.model.desc': 'Screen high-risk tool calls with local rules, then ask a second model to judge the matches. A review can only make execution stricter.',
  'review.section.filter': 'Filtering & blocking strategy',
  'review.section.filter.desc': 'Pre-filter calls before invoking review to skip read-only queries or directly deny dangerous deletion commands.',
  'review.section.rules': 'Custom regex rules',
  'review.section.rules.desc': 'Custom regular expression patterns for high-risk interception, read-only bypass, and absolute deletion.',
  'review.section.verdicts': 'Recent verdicts',
  'review.section.verdicts.desc': 'Audit log and reasons for verdicts made by the second reviewer model and local rules on historical tool calls.',
  'review.resetCurrentRule': 'Reset current category to default',

  // Explorer
  'explorer.side': 'Side',
  'explorer.side.left': 'Left',
  'explorer.side.right': 'Right',
  'explorer.defaultOpen': 'Open by default',
  'explorer.openLinksInPanel': 'Open file links in side panel',
  'explorer.openLinksInPanel.hint': 'When clicking file links in messages or tool cards, open them directly in the side panel editor instead of launching external applications.',
  'explorer.openInSidePanel': 'Open in side panel',
  'explorer.gitignore': 'Respect .gitignore',
  'explorer.preview': 'Preview',
  'explorer.changes': 'Review',
  'explorer.files': 'Files',
  'explorer.noRepo': 'Not a git repository.',
  'explorer.noChanges': 'No uncommitted changes.',
  'explorer.workspace': 'workspace',
  'explorer.up': '‹ up',
  'explorer.truncated': '…more entries than the configured cap; refine the path to see them.',
  'explorer.noDiff': '(no textual diff)',
  'explorer.diffFailed': 'Could not read the diff: {message}',
  'explorer.title': 'Project',
  'explorer.off': 'The explorer is switched off.',
  'explorer.resize': 'Resize the panel',
  'explorer.openEditor': 'Open in',
  'explorer.openLabel': 'Open',
  'explorer.openEditorFailed': 'Could not open: {message}',
  'explorer.openWith.explorer': 'File Explorer',
  'explorer.openWith.vscode': 'VS Code',
  'explorer.openWith.idea': 'IntelliJ IDEA',
  'explorer.viewFailed': 'Could not read the file: {message}',
  'explorer.truncatedFile': '…only the first {lines} lines are shown.',
  'explorer.close': 'Close the preview',
  'explorer.workspacePick': 'Workspace',
  'explorer.views': 'Views',
  'explorer.newTab': 'Add a view',
  'explorer.closeTab': 'Close tab',
  'explorer.unmodified': '{n} unmodified lines',
  'explorer.filterAll': 'All',
  'explorer.filterStaged': 'Staged',
  'explorer.filterUnstaged': 'Unstaged',
  'explorer.reviewGroup': 'Group by folder',
  'explorer.reviewFlat': 'Flat list',

  // Terminal
  'section.terminal': 'Terminal',
  'section.terminal.desc': 'A real shell in the side panel. New tabs open from the + menu; each runs its own PTY on the machine running the harness.',
  'terminal.tab': 'Terminal',
  'terminal.newTab': 'New terminal',
  'terminal.connecting': 'Connecting…',
  'terminal.exit': 'Process exited with code {code}.',
  'terminal.exitShort': 'Exited ({code})',
  'terminal.restart': 'Restart',
  'terminal.disconnected': 'The connection dropped.',
  'terminal.failed': 'Terminal unavailable: {message}',
  'terminal.shell': 'Shell',
  'terminal.shell.hint': "'auto' follows the platform default (PowerShell on Windows, $SHELL on macOS/Linux). Presets or an absolute executable path also work.",
  'terminal.shell.auto': 'Auto',
  'terminal.shell.unavailable': 'not installed',
  'terminal.shellArgs': 'Shell arguments',
  'terminal.shellArgs.hint': 'Extra arguments appended when the shell starts, space separated.',
  'terminal.scrollback': 'Scrollback (lines)',
  'terminal.scrollback.hint': 'Output kept per terminal on the server, replayed when the tab reattaches after a refresh or a switch.',
  'terminal.ptyMissing': 'The PTY module (@lydell/node-pty) failed to load on the server, so terminals cannot start: {message}',

  // Change kinds
  'change.untracked': 'untracked',
  'change.modified': 'modified',
  'change.modifiedStaged': 'modified, staged',
  'change.added': 'added',
  'change.deleted': 'deleted',
  'change.renamed': 'renamed',
  'change.copied': 'copied',
  'change.conflicted': 'conflicted',
  'change.typeChanged': 'type changed',
  'change.staged': 'staged',
  'change.changed': 'changed',

  // Sessions
  'sessions.gc': 'Collect unused attachments',
  'sessions.gc.hint': 'On permanent delete, scan every remaining session before removing an image blob. Slow on a large history.',
  'sessions.updatedAt': 'updated {when}',
  'sessions.trashCount': 'Recycle bin ({n})',
  'sessions.trashButton': 'Recycle bin',
  'sessions.trashEmpty': 'The recycle bin is empty.',
  'sessions.emptyTrash': 'Empty recycle bin',
  'sessions.emptyTrashConfirm': 'Delete every archived session for good? This cannot be undone.',
  'sessions.deleteForever': 'Delete forever',
  'sessions.deleteForeverConfirm': 'Delete “{title}” for good? This cannot be undone.',
  'sessions.off': 'Session administration is switched off.',

  // Plugins
  'plugins.list': 'Installed plugins and rescue',
  'plugins.none': 'No third-party plugins are installed.',
  'plugins.quarantined': 'quarantined',
  'plugins.quarantinedCount': '{n} plugin(s) are quarantined and will not load on the next start.',
  'plugins.enable': 'Enable',
  'plugins.enableAll': 'Enable all',
  'plugins.disable': 'Disable',
  'plugins.rescueTitle': 'Fault Rescue Mechanism',
  'plugins.rescueBody': 'When a third-party plugin causes Harness startup to fail, use these recovery mechanisms:',
  'plugins.rescueWebTitle': '1. Web Boot Error Rescue (In-Browser)',
  'plugins.rescueWebDesc': 'If frontend plugin boot gets stuck on the error card, an emergency panel appears automatically to quarantine the faulty plugin or enter Safe Mode with one click.',
  'plugins.rescueCliTitle': '2. Rescue CLI (Out-of-Process Fallback)',
  'plugins.rescueCliDesc': 'If the Node backend process exits during startup, run the standalone CLI commands in your terminal (zero external dependencies):',
  'plugins.rescueFile': 'Quarantine records are written to {file} with top-layer priority. Your custom entries in that file remain intact.',
  'plugins.off': 'Plugin safety is switched off.',


  // Checkpoints
  'cp.snapshotOn': 'Snapshot on',
  'cp.snapshotOn.turn': 'Each turn',
  'cp.snapshotOn.tool': 'Each mutating tool call',
  'cp.retention': 'Retention (days)',
  'cp.retention.hint': 'Checkpoints older than this are pruned when the plugin loads. 0 keeps everything.',
  'cp.maxSize': 'Skip files larger than (MB)',
  'cp.list': 'Checkpoints and restore',
  'cp.take': 'Take a checkpoint now',
  'cp.none': 'No checkpoints have been taken for this workspace yet. One is recorded before the agent\'s first file change.',
  'cp.diff': 'Diff',
  'cp.restore': 'Restore…',
  'cp.restoreAnswer': 'Restore workspace to before this answer',
  'cp.rollback': 'Rollback',
  'cp.restoreAnswerUnavailable': 'This answer made no restorable file changes',
  'cp.restoreAnswerTitle': 'Restore this answer’s changes?',
  'cp.restoreAnswerHint': 'Restore workspace files to the checkpoint before this answer’s first file change. The conversation stays intact. {n} path(s) currently differ from it.',
  'cp.restoreAnswerNoop': 'No tracked workspace files differ from this checkpoint. Git metadata (.git) is intentionally excluded and cannot be restored here.',
  'cp.restoreWorkspace': 'Workspace: {path}',
  'cp.baseline': 'baseline',
  'cp.noLabel': '(no label)',
  'cp.restoreTitle': 'Restore checkpoint {id}?',
  'cp.restoreNoop': 'Your working tree already matches this checkpoint; nothing would change.',
  'cp.restoreCount': '{n} file(s) would be written or removed. Your own git history, index, and stashes are not touched.',
  'cp.unprotected': '{n} of them are not tracked by your git, so this working copy is the only one:',
  'cp.andMore': '…and {n} more',
  'cp.restoring': 'Restoring…',
  'cp.forkFailed': 'Workspace files are restored, but switching the chat to the branch failed: {message}',
  'cp.forkUnavailable': 'Workspace files are restored, but the chat branch point could not be located. Reopen the conversation to continue.',
  'turn.filesChanged': '{n} file(s) changed',
  'turn.checking': 'Checking for file changes…',
  'turn.toggleList': 'Toggle the file list',
  'turn.review': 'Review',
  'turn.open': 'Open',
  'turn.edit': 'Edit',
  'turn.copy': 'Copy',
  'turn.copied': 'Copied',
  'turn.undo': 'Undo',
  'turn.undoTitle': 'Undo this turn?',
  'turn.undoHint': 'Workspace files go back to before this turn’s {n} file change(s), and this turn and everything after it leave the branch (the original session keeps every record).',
  'turn.editTitle': 'Edit this turn’s question',
  'turn.editHint': 'Sending restores the files to before this turn, removes this turn and everything after it from the branch, then re-answers with the edited question.',
  'turn.sendEdit': 'Send & re-answer',
  'turn.stillRunning': 'This turn is still running',
  'turn.firstTurnNoFork': 'The session’s first turn has no earlier branch point to cut at, so it cannot be undone or edited',
  'turn.firstTurnUndo': 'This is the session’s first turn — there is no earlier point to cut the chat at. The workspace files are restored, and a new session on the same project is opened.',
  'turn.trashFailed': 'Files restored and a new session opened, but the original session could not be archived into the recycle bin (session admin may be off).',
  'turn.editSendFailed': 'The branch was created, but re-sending failed: {message}. You can send the question manually in the new session.',
  'cp.restored': 'Restored. A checkpoint of the previous state was taken first, so this is undoable.',
  'cp.previewFailed': 'Could not work out what a restore would change: {message}',
  'cp.emptyDiff': '(this checkpoint changed nothing textual)',
  'cp.checkpointN': 'Checkpoint {id}',
  'cp.off': 'Checkpoints are switched off.',
  'cp.justNow': 'just now',
  'cp.minsAgo': '{n} min ago',
  'cp.hoursAgo': '{n} h ago',
  'cp.unknownTime': 'unknown time',

  // Git tabs and sections
  'tab.git': 'Git',
  'section.git': 'Git version control',
  'section.git.desc': 'Commit, push, AI-assisted commit message, branch management, and session-bound worktrees.',
  'git.section.commit': 'AI Commit generation',
  'git.section.binding': 'Session-branch binding',
  'git.section.worktree': 'Worktree isolation',
  'git.section.push': 'Push & remote safety',

  // Git settings options
  'git.model': 'Model',
  'git.model.hint': 'Model used to generate commit messages from git diff. Empty follows current chat session model.',
  'git.model.followSession': 'Follow active session model (Default)',
  'git.commitStyle': 'Commit format style',
  'git.commitStyle.conventional': 'Conventional Commits (feat/fix standard)',
  'git.commitStyle.simple': 'Single-line summary',
  'git.commitStyle.detailed': 'Detailed change points',
  'git.commitLanguage': 'Commit language',
  'git.commitLanguage.zh': 'Simplified Chinese (简体中文)',
  'git.commitLanguage.en': 'English (en)',
  'git.commitLanguage.auto': 'Follow UI language',
  'git.autoStageAll': 'Auto stage all when empty',
  'git.autoStageAll.hint': 'When staging area is empty, automatically stage all modified files before generating commit or committing.',
  'git.sessionBinding': 'Session binding mode',
  'git.sessionBinding.strict': 'Strict: bound on creation and locked in session (Recommended)',
  'git.sessionBinding.prompt': 'Prompt: prompts on creation, allow unlock',
  'git.sessionBinding.off': 'Off: unrestricted',
  'balance.stale': 'Outdated balance',
  'sessions.gc.unsupported': 'Unavailable: this host does not expose a safe attachment deletion API. Attachments are retained.',
  'git.autoAlignBranch': 'Auto align repository branch',
  'git.autoAlignBranch.hint': 'When switching to a session, auto checkout its bound branch if in the same repository.',
  'git.worktreeDirPattern': 'Worktree directory pattern',
  'git.worktreeDirPattern.hint': 'Default path template for new worktrees. Variables: {repo}, {branch}.',
  'git.worktreeAutoRegister': 'Auto register as workspace',
  'git.worktreeAutoRegister.hint': 'Automatically register newly created worktrees in DSH workspace registry.',
  'git.pushAutoSetUpstream': 'Auto set upstream (--set-upstream)',
  'git.pushAutoSetUpstream.hint': 'Automatically attach -u origin <branch> on first push if remote tracking is not set.',
  'git.pushTimeout': 'Push timeout (seconds)',
  'git.pushTimeout.hint': 'Maximum wait time in seconds for git push network operations.',

  // Git review and explorer UI
  'git.commit': 'Commit',
  'git.commitAndPush': 'Commit & Push',
  'git.push': 'Push',
  'git.pushCount': 'Push (↑{n})',
  'git.stageAll': 'Stage all',
  'git.unstageAll': 'Unstage all',
  'git.stagedChanges': 'Staged Changes',
  'git.unstagedChanges': 'Unstaged Changes',
  'git.changes': 'Changes',
  'git.unstageAllChanges': 'Unstage all changes',
  'git.stageAllChanges': 'Stage all changes',
  'git.discardAllChanges': 'Discard all changes',
  'git.noStagedChanges': 'No staged changes',
  'git.noUnstagedChanges': 'No unstaged changes',
  'git.stageFile': 'Stage file',
  'git.unstageFile': 'Unstage file',
  'git.discardChange': 'Discard changes',
  'git.stageFolder': 'Stage folder',
  'git.unstageFolder': 'Unstage folder',
  'git.discardFolder': 'Discard folder changes',
  'git.confirmDiscardAll': 'Are you sure you want to discard ALL unstaged changes in {n} files?',
  'git.confirmDiscardFile': 'Are you sure you want to discard unstaged changes in "{file}"?',
  'git.confirmDiscardFolder': 'Are you sure you want to discard unstaged changes in {n} files under "{folder}"?',
  'git.confirmDiscardAction': 'Discard Changes',
  'git.discardIrreversible': 'This action cannot be undone. Unstaged changes in your working tree will be permanently deleted.',
  'git.aiGenerate': '✨ AI Generate',
  'git.aiGenerating': 'Generating...',
  'git.commitPlaceholder': 'Enter commit message or click ✨ AI Generate...',
  'git.stagedCount': '{n} staged',
  'git.unstagedCount': '{n} unstaged',
  'git.committed': 'Committed successfully: {hash}',
  'git.pushed': 'Pushed successfully',
  'git.pushFailed': 'Push failed: {message}',
  'git.commitFailed': 'Commit failed: {message}',
  'git.generateFailed': 'Failed to generate commit message: {message}',
  'git.branches': 'Branches',
  'git.worktrees': 'Worktrees',
  'git.searchBranches': 'Search branches...',
  'git.currentBranch': 'Current',
  'git.lockedSession': 'Locked to this session',
  'git.cannotSwitchInSession': 'This session is locked to branch [{branch}]. To work on another branch, open or create a session.',
  'git.openSession': 'Open session',
  'git.newSessionForBranch': 'New session for branch',
  'git.createBranch': 'New branch',
  'git.createBranchBtn': 'Create & checkout',
  'git.branchName': 'Branch name',
  'git.startPoint': 'Based on (optional)',
  'git.createWorktree': 'New Worktree',
  'git.worktreePath': 'Directory path',
  'git.useWorktree': 'Use Worktree',
  'git.openInEditor': 'Open in editor',
  'git.removeWorktree': 'Remove',
  'git.removeWorktreeConfirm': 'Remove worktree at {path}? Files in that directory will be deleted.',
  'git.detached': 'detached at {sha}',
  'git.sessionPicker.title': 'Work Branch',
  'git.sessionPicker.hint': 'Select or create a branch for this session. It will be locked on your first message.',
  'git.sessionPicker.lockNotice': 'Locked: {branch}',
  'git.localBranches': 'Local branches',
  'git.remoteBranches': 'Remote branches',
  'git.checkoutBranch': 'Checkout',
  'git.checkoutSuccess': 'Switched to branch {branch}',
  'git.sessionLockedTooltip': 'Current branch: {branch} (Session locked)',
  'git.worktreeMain': 'Main',
  'git.worktreeDropdown': 'Workspaces & Worktrees',
  'git.branchDropdown': 'Git Branches',
  'git.mainWorkspace': 'Main Workspace (Default)',
  'git.otherWorktrees': 'Linked Worktrees',
  'git.noOtherWorktrees': 'No other worktrees',
  'git.boundToWorktreeNotice': 'Currently in a linked worktree. Branch is fixed to [{branch}]. To switch branches, select the Main Workspace.',
  'git.checkedOutInWorktree': 'In use by worktree',
  'git.createWorktreeAction': 'Create Worktree...',
  'git.createBranchAction': 'Create Branch...',
  'git.autoRegisterDsh': 'Auto-register as DeepSeek workspace (Recommended)',
  'git.checkoutAfterCreate': 'Checkout branch after creation',
  'git.useExistingBranch': 'Use existing branch',
  'git.createNewBranch': 'Create new branch',
  'git.worktreePrefix': 'Worktree:',
  'git.worktreeOfPrefix': '{name} Worktree:',
  'git.worktreeTag': '{name} Worktree',
  'git.mainWorkspacePrefix': 'Main:',
  'git.branchPrefix': 'Branch:',
  'git.notRepository': 'Current workspace is not a Git repository',
  'git.initRepository': 'Initialize Git repository',
  'git.initRepositoryHint': 'Run git init in this workspace',
  'git.initSuccess': 'Git repository initialized',
  'git.initFailed': 'Failed to initialize Git repository: {message}',
} as const

export type LocaleKey = keyof typeof en

export const zh: Record<LocaleKey, string> = {
  'section.images': '输入框图片',
  'section.images.desc': '在 + 菜单最上面加一条图片入口，草稿图片可拖动改顺序。',

  // Settings tabs
  'tab.input': '输入与模型',
  'tab.balance': '余额',
  'tab.review': '命令审核',
  'tab.files': '文件',
  'tab.sessions': '会话',
  'tab.plugins': '故障救援',

  // Balance extras
  'balance.peak': '高峰',
  'balance.offPeak': '低谷',
  'balance.poll': '轮询间隔（秒）',
  'balance.poll.hint': '余额文字每 N 秒自动刷新。0 表示不轮询。',
  'balance.peakWindows': '高峰时段（北京时间）',
  'balance.peakWindows.hint': '官方时段换算为北京时间：周一至周五 09:00-12:00 与 14:00-18:00，窗口外半价。逗号分隔 HH:MM-HH:MM。',
  'balance.peakWeekdays': '周末不计高峰',
  'balance.peakWeekdays.hint': '官方规则：周六周日全天按低谷计价。',
  'balance.badge.title': 'DeepSeek 余额 · 高峰窗口 {windows}（北京时间）· key 来源 {source}',
  'section.effort': '推理强度',
  'section.effort.desc': '为第三方（pi-ai）线路的每个模型声明推理档位，声明后输入框自带的强度选择器就会对这些模型出现。',
  'section.balance': 'DeepSeek 余额',
  'section.balance.desc': '读取已配置的 DeepSeek 官方 API 密钥的账户余额。密钥不会进入浏览器。',
  'section.review': '命令审核',
  'section.review.desc': '先用本地规则筛查高危工具调用，命中的再交给第二个模型判断。审核只会让调用更严格，不会放宽。',
  'section.explorer': '项目浏览',
  'section.explorer.desc': '工作区目录树与未提交的改动。只读：背后每条 git 命令都是查询。',
  'section.sessions': '会话',
  'section.sessions.desc': '回收站在侧栏底部、「设置」旁边。关闭会话管理会隐藏回收站，撤销/编辑时原会话也会留在列表里。',
  'section.plugins': '故障救援',
  'section.plugins.desc': '管理插件隔离名单与故障自愈命令，确保 Harness 在任何情况下都能安全启动。',
  'section.checkpoints': '检查点',
  'section.checkpoints.desc': '通过影子 git 仓库做按会话回滚。你项目自己的 git 历史、索引和 stash 绝不会被触碰。',

  'common.enabled': '启用',
  'common.on': '开',
  'common.off': '关',
  'common.refresh': '刷新',
  'common.clear': '清空',
  'common.close': '关闭',
  'common.cancel': '取消',
  'common.save': '保存',
  'common.reset': '重置',
  'common.delete': '删除',
  'common.restore': '恢复',
  'common.yes': '确定',
  'common.no': '不了',
  'common.loading': '加载中…',
  'common.days': '天',
  'common.readonly': '当前部署没有挂载设置提供者，改动无法保存。下面显示的是组合传入的值。',

  'images.button': '+ 菜单里的图片入口',
  'images.button.hint': '在 + 菜单的第一条加上「添加图片」。',
  'images.drag': '拖动改顺序',
  'images.drag.hint': '拖动缩略图，或用它的 ‹ › 按钮。消息发送中会拒绝改顺序。',
  'images.add': '添加图片',
  'images.pickHint': '为这条消息附加图片',
  'files.attach': '添加附件',
  'files.attachHint': '从本机选一个文件附加进来',
  'files.notImage': '{name} 不是图片，已把它的文本内容加进消息。',
  'files.unreadable': '{name} 无法附加：既不是图片，也不是文本。',
  'images.remove': '移除 {name}',
  'images.earlier': '把 {name} 往前移',
  'images.later': '把 {name} 往后移',
  'images.busy': '输入框正在发送，图片顺序保持原样。',
  'images.drop': '拖入图片即可附加',
  'images.dropLimits': '最多 {count} 张，每张 {size}',
  'images.rail': '草稿图片',
  'images.railMissing': '图片栏未挂载',

  'effort.models': '模型与档位',
  'effort.defaultFull': '默认完整推理档位',
  'effort.defaultFull.hint': '没有显式覆盖的模型默认拥有 off、minimal、low、medium、high、xhigh、max 全部档位。',
  'effort.none': '还没有配置第三方（pi-ai）供应商。请先在「模型」页面添加。',
  'effort.noEfforts': '无档位',
  'effort.fromAdapter': '来自适配器：{list}',
  'effort.declare': '声明',
  'effort.edit': '编辑',
  'effort.notLoaded': '已配置，未加载',
  'effort.level': '档位',
  'effort.sentAs': '实际发送',
  'effort.explain': '左列是你在输入框里看到的选项，右列是实际发给供应商的值 —— 只有 off 可以留空，表示什么都不发。',
  'effort.add': '添加：',
  'effort.remove': '移除',
  'effort.noReasoning': '不推理',
  'effort.noReasoning.hint': '记录这个模型完全不做推理',
  'effort.reset.hint': '移除本插件的覆盖，回到适配器自己的说法',
  'effort.sendNothing': '（什么都不发）',
  'effort.required': '必填',
  'effort.readonly': '没有挂载设置提供者，推理档位无法保存。',

  // 图片能力声明
  'vision.on': '支持图片 ✓',
  'vision.off': '支持图片 ✕',
  'vision.defaultAll': '默认支持图片',
  'vision.defaultAll.hint': '没有显式声明输入模态的模型默认配置为同时支持文字和图片。',
  'vision.hint': '该模型是否接受图片附件。模型未声明图片模态时，宿主会拒绝发送图片。',

  // 输入框模型菜单
  'section.modelPicker': '模型菜单',
  'section.modelPicker.desc': '输入框里的模型列表，按供应商分组折叠，并支持按名称筛选。',
  'modelPicker.collapse': '分组可折叠',
  'modelPicker.collapse.hint': '关闭后所有供应商始终展开，和自带菜单一样。',
  'picker.chooseModel': '选择模型',
  'picker.triggerAria': '选择模型，当前 {model}',
  'picker.triggerAriaEffort': '选择模型，当前 {model}，推理等级 {effort}',
  'picker.menuAria': '模型与推理等级',
  'picker.model': '模型',
  'picker.effort': '推理等级',
  'picker.providerDefault': '默认',
  'picker.loading': '正在刷新模型列表…',
  'picker.actionFailed': '模型操作失败：{message}',
  'picker.reload': '重新加载',
  'picker.groupFailed': '{name} 加载失败：{message}',
  'picker.noModels': '没有可用的模型。',
  'picker.noMatch': '没有匹配的模型。',
  'picker.noEfforts': '这个模型没有暴露推理档位。',
  'picker.filter': '筛选模型',
  'picker.collapseAll': '全部折叠',
  'picker.expandAll': '全部展开',

  'effortPicker.label': '推理强度',
  'effortPicker.none': '这个模型没有暴露推理档位。',
  'effortPicker.default': '供应商默认',
  'effortPicker.declaring': '正在声明…',
  'effortPicker.declareStandard': '启用标准档位（关/低/中/高）',
  'effortPicker.notDescribed': '已安装的模型目录里没有这个路由，所以不能在这里为它声明档位。先在「模型」页面把这个模型写进供应商的模型列表。',
  'effortPicker.failed': '模型拒绝了这个推理档位。',

  'balance.currency': '币种',
  'balance.total': '总额',
  'balance.granted': '赠送',
  'balance.toppedUp': '充值',
  'balance.badge': '输入框余额标记',
  'balance.badge.hint': '在输入框里、模型选择器左边显示一个紧凑的余额标记。',
  'balance.reading': '正在读取余额…',
  'balance.unavailable': 'DeepSeek 报告这个账户当前无法服务请求。',
  'balance.noRows': '接口没有返回任何余额行。',
  'balance.keyFrom': '密钥来自 {source}',

  'review.mode': '模式',
  'review.mode.rules': '仅本地规则',
  'review.mode.rulesLlm': '规则命中后交模型',
  'review.mode.all': '所有覆盖的调用',
  'review.provider': '审核供应商',
  'review.provider.hint': '审核模型走哪个供应商路由。填「模型」页里已配置的任意路由。',
  'review.model': '审核模型',
  'review.model.hint': '审核时询问的模型 id。按回车或点击别处保存。',
  'review.timeout': '审核超时（毫秒）',
  'review.timeout.hint': '等待审核结论的时长，超时后按下面那条规则处理（1000–120000 毫秒）。',
  'review.deadline': '{ms} 毫秒超时',
  'review.onFailure': '审核失败时',
  'review.onFailure.hint': '审核模型超时、报错或没有凭据时怎么处理。',
  'review.onFailure.ask': '询问用户',
  'review.onFailure.deny': '拒绝调用',
  'review.onFailure.allow': '放行并记录',
  'review.tools': '受审工具',
  'review.tools.hint': '会被审核的工具名；在 settings.yaml 里改这个列表。',
  'review.writeOnly': '只审查写操作',
  'review.writeOnly.hint': '跳过可识别的静态只读命令；全量审核模式不跳过只读调用，并发安全元数据不作为豁免依据。',
  'review.denyPatterns': '高风险规则',
  'review.denyPatterns.hint': '每行一个正则表达式。命中规则或不能确认只读的调用会升级人工或交给模型判断。',
  'review.readPatterns': '只读命令规则',
  'review.readPatterns.hint': '每行一个匹配整条命令的正则表达式，用于收窄内置静态只读识别范围；执行其他程序的参数不能被豁免。',
  'review.absoluteDelete': '绝对禁止删除',
  'review.absoluteDelete.hint': '识别到删除操作后立即拒绝，模型和人工审批都不能覆盖。',
  'review.deleteCommand': '删除命令',
  'review.gitPushCommand': 'git push（推送）命令',
  'review.commandPolicy.deny': '绝对禁止',
  'review.commandPolicy.ask': '询问用户',
  'review.commandPolicy.allow': '允许',
  'review.commandPolicy.expected': '按预期审核（默认）',
  'review.mode.default': '默认',
  'review.mode.expected.label': '按预期审核',
  'review.mode.expected.hint': '针对其他命令，结合来源可核实的用户任务、实际参数、工作目录和目标路径，检查引号、空格、变量及操作范围是否出错。符合预期可通过，依据不足或目标重大且未明确时询问，明确的严重错误拒绝。开启只审查写操作时，可识别的只读命令可跳过。删除与推送始终走各自的独立策略。',
  'review.mode.rules+llm.label': '规则初筛 + 模型复审',
  'review.mode.rules+llm.hint': '针对其他命令，本地先识别只读操作与风险特征，风险命中或无法确认只读的操作交给 AI 做一般安全审核，不默认重建任务预期。删除与推送只执行各自的专项策略，不重复进入此模式。',
  'review.mode.rules-only.label': '仅本地规则',
  'review.mode.rules-only.hint': '其他命令不调用 AI，依据本地规则处理；可识别的只读操作可通过，无法确认的写操作转人工确认。如果删除或推送单独选择了按预期审核，该类别仍独立调用 AI，不受此模式限制。',
  'review.mode.all.label': '全量模型审查',
  'review.mode.all.hint': '将受审范围内的其他操作全部交给 AI 做一般安全审核，包括只读操作；会增加请求次数与等待时间，不代表全部拒绝。删除与推送仍只执行各自的专项策略，不重复进入全局审核。',
  'review.deletePolicy.deny.hint': '识别到删除操作后直接拒绝，不进入 AI 审核或人工确认。即使删除符合当前任务也不能放行；复合命令中的其他操作不能覆盖此拒绝。',
  'review.deletePolicy.ask.hint': '每次识别到删除操作都必须由你确认，AI 不能自动批准，也不再进入全局审核模式。复合命令中的其他操作仍会分别检查，宿主自身的权限限制仍然有效。',
  'review.deletePolicy.expected.hint': '独立检查删除对象与用户任务是否一致，重点关注路径拆分、空变量、通配符和误指向项目根目录。目标明确且符合预期时可以通过；依据不足、关键目标或路径无法核实时询问；有具体证据的严重错误拒绝。即使全局是仅本地规则，此项仍调用 AI。',
  'review.deletePolicy.allow.hint': '直接允许已识别的删除操作，跳过本插件的 AI 审核和人工询问，不受全局审核模式影响。复合命令中夹带的其他操作仍分别审核，宿主权限限制仍然有效；无法可靠拆分的语法会转人工确认，不扩大为整条命令的放行。',
  'review.gitPushPolicy.deny.hint': '识别到 Agent 执行 git push 后直接拒绝，普通推送、强制推送和标签推送均受限制，AI 与人工确认都不能覆盖。此设置不改变你手动点击 Git 推送按钮的行为。',
  'review.gitPushPolicy.ask.hint': '每次识别到 Agent 推送都必须由你确认，不能因为代码已修改或测试通过就自动推送。此类别不再进入全局审核模式；复合命令的其他操作与宿主权限仍需分别检查。',
  'review.gitPushPolicy.expected.hint': '独立检查用户是否明确要求推送，以及远端、分支或引用范围、强制参数是否符合要求。修改代码或本地提交不代表授权推送；授权不明、隐式推送目标未核实时询问；有证据的严重目标错误拒绝。此项不受全局审核模式影响。',
  'review.gitPushPolicy.allow.hint': '直接允许已识别的 Agent 推送，跳过本插件的 AI 审核和人工询问，不受全局审核模式影响，因此 Agent 可以自动推送。复合命令中的其他操作仍分别审核，宿主权限仍然有效；无法可靠拆分的语法会转人工确认。',
  'review.deletePatterns': '删除操作规则',
  'review.deletePatterns.hint': '每行一个正则表达式，匹配工具标识和执行语法，不把搜索文字或补丁新增内容当作删除命令。',
  'review.verdicts': '最近的判定',
  'review.empty': '还没有审核过任何命令。',
  'review.count': '最近 {n} 条判定。',
  'review.matched': '命中规则：',
  'review.off': '命令审核已关闭。',
  'review.modelPick': '审查模型',
  'review.modelPick.hint': '可选择固定审查模型，或自动跟随发起调用的会话模型。安全审核提示词独立保留；无法获取会话模型时按审核失败策略处理。',
  'review.model.followSession': '自动跟随当前会话模型（默认）',
  'review.autoChip': '自动审核',
  'review.autoChip.hint': '所有写命令先交审查模型判定，模型无法判断风险时再询问用户。',
  'review.autoReview': '聊天自动审核',
  'review.autoReview.hint': '开启后，会话中的命令才会真正触发规则与模型审查；关闭时处于暂停放行状态。',
  'review.section.model': '审核模式与模型策略',
  'review.section.model.desc': '先用本地规则筛查高危工具调用，命中的再交给第二个模型判断。审核只会让调用更严格，不会放宽。',
  'review.section.filter': '过滤与阻断策略',
  'review.section.filter.desc': '在调用审查前进行前置规则过滤，可跳过只读查询或直接拒绝危险删除操作。',
  'review.section.rules': '自定义正则规则',
  'review.section.rules.desc': '自定义高危拦截、只读放行与绝对删除特征的正则表达式规则词表。',
  'review.section.verdicts': '审核判定记录',
  'review.section.verdicts.desc': '第二审查模型与本地规则对历史工具调用执行审核判定的日志记录与处置理由。',
  'review.resetCurrentRule': '恢复当前分类默认规则',

  'explorer.side': '停靠位置',
  'explorer.side.left': '左侧',
  'explorer.side.right': '右侧',
  'explorer.defaultOpen': '默认展开',
  'explorer.openLinksInPanel': '点击文件链接在侧边栏打开',
  'explorer.openLinksInPanel.hint': '点击对话消息或工具调用中的文件链接时，直接在侧边栏代码编辑器中打开，而非唤起系统外部应用。',
  'explorer.openInSidePanel': '在侧边栏打开',
  'explorer.gitignore': '遵循 .gitignore',
  'explorer.preview': '预览',
  'explorer.changes': '审查',
  'explorer.files': '文件',
  'explorer.noRepo': '不是 git 仓库。',
  'explorer.noChanges': '没有未提交的改动。',
  'explorer.workspace': '工作区',
  'explorer.up': '‹ 上一级',
  'explorer.truncated': '…条目超过配置上限，缩小路径范围才能看到其余的。',
  'explorer.noDiff': '（没有文本差异）',
  'explorer.diffFailed': '读不出差异：{message}',
  'explorer.title': '项目',
  'explorer.off': '项目浏览已关闭。',
  'explorer.resize': '调整面板宽度',
  'explorer.openEditor': '打开方式',
  'explorer.openLabel': '打开',
  'explorer.openEditorFailed': '打不开：{message}',
  'explorer.openWith.explorer': '资源管理器',
  'explorer.openWith.vscode': 'VS Code',
  'explorer.openWith.idea': 'IntelliJ IDEA',
  'explorer.viewFailed': '读不出这个文件：{message}',
  'explorer.truncatedFile': '…只显示前 {lines} 行。',
  'explorer.close': '关闭预览',
  'explorer.workspacePick': '工作区',
  'explorer.views': '视图',
  'explorer.newTab': '新建标签页',
  'explorer.closeTab': '关闭标签页',
  'explorer.unmodified': '{n} 行未修改',
  'explorer.filterAll': '全部',
  'explorer.filterStaged': '已暂存',
  'explorer.filterUnstaged': '未暂存',
  'explorer.reviewGroup': '按文件夹分组',
  'explorer.reviewFlat': '平铺列表',

  // Terminal
  'section.terminal': '终端',
  'section.terminal.desc': '侧边栏里的真实终端。在 + 菜单里新建标签页，每个标签页是运行 Harness 的机器上的一个独立 PTY 会话。',
  'terminal.tab': '终端',
  'terminal.newTab': '新建终端',
  'terminal.connecting': '连接中…',
  'terminal.exit': '进程已退出，退出码 {code}。',
  'terminal.exitShort': '已退出（{code}）',
  'terminal.restart': '重新启动',
  'terminal.disconnected': '连接已断开。',
  'terminal.failed': '终端不可用：{message}',
  'terminal.shell': 'Shell',
  'terminal.shell.hint': 'auto 跟随平台默认（Windows 用 PowerShell，macOS/Linux 用 $SHELL）；也可以选预设，或填可执行文件的绝对路径。',
  'terminal.shell.auto': '自动',
  'terminal.shell.unavailable': '未安装',
  'terminal.shellArgs': 'Shell 启动参数',
  'terminal.shellArgs.hint': '启动 Shell 时追加的参数，空格分隔。',
  'terminal.scrollback': '回滚行数',
  'terminal.scrollback.hint': '每个终端在服务端保留的输出行数，页面刷新或切回标签页时用于恢复画面。',
  'terminal.ptyMissing': '服务端 PTY 模块（@lydell/node-pty）加载失败，无法启动终端：{message}',

  'change.untracked': '未跟踪',
  'change.modified': '已修改',
  'change.modifiedStaged': '已修改并暂存',
  'change.added': '新增',
  'change.deleted': '已删除',
  'change.renamed': '已重命名',
  'change.copied': '已复制',
  'change.conflicted': '有冲突',
  'change.typeChanged': '类型变更',
  'change.staged': '已暂存',
  'change.changed': '有变动',

  'sessions.gc': '清理无用附件',
  'sessions.gc.hint': '永久删除时，先扫描其余所有会话再移除图片文件。历史很大时会慢。',
  'sessions.updatedAt': '更新于 {when}',
  'sessions.trashCount': '回收站（{n}）',
  'sessions.trashButton': '回收站',
  'sessions.trashEmpty': '回收站是空的。',
  'sessions.emptyTrash': '清空回收站',
  'sessions.emptyTrashConfirm': '永久删除所有已归档会话？此操作无法撤销。',
  'sessions.deleteForever': '永久删除',
  'sessions.deleteForeverConfirm': '永久删除「{title}」？此操作无法撤销。',
  'sessions.off': '会话管理已关闭。',

  'plugins.list': '已装插件与救援',
  'plugins.none': '没有安装第三方插件。',
  'plugins.quarantined': '已隔离',
  'plugins.quarantinedCount': '有 {n} 个插件被隔离，下次启动不会加载。',
  'plugins.enable': '启用',
  'plugins.disable': '禁用',
  'plugins.enableAll': '启用所有被隔离的插件',
  'plugins.rescueTitle': '故障救援机制',
  'plugins.rescueBody': '当第三方插件异常导致系统启动失败时，可通过以下脱困方案快速恢复：',
  'plugins.rescueWebTitle': '1. 网页引导错误页急救（开箱即用）',
  'plugins.rescueWebDesc': '若前端加载插件停滞在错误引导页，页面会自动呼出急救面板，支持在浏览器中一键隔离故障插件或开启安全模式。',
  'plugins.rescueCliTitle': '2. 终端脱困 CLI（进程外兜底）',
  'plugins.rescueCliDesc': '若后台 Node 进程在启动初期直接崩溃退出，可直接在终端执行独立急救命令（零外部依赖）：',
  'plugins.rescueFile': '隔离配置自动持久化到 {file}。该层级位于所有插件配置之后，享有最高优先级覆盖权，重启即生效。用户在该文件中自行编写的其他配置不受影响。',
  'plugins.off': '插件安全已关闭。',


  'cp.snapshotOn': '快照时机',
  'cp.snapshotOn.turn': '每轮对话',
  'cp.snapshotOn.tool': '每次改动文件的工具调用',
  'cp.retention': '保留期（天）',
  'cp.retention.hint': '插件加载时会清掉早于这个期限的检查点。0 表示全部保留。',
  'cp.maxSize': '跳过大于（MB）',
  'cp.list': '检查点与恢复',
  'cp.take': '立即创建检查点',
  'cp.none': '这个工作区还没有任何检查点。agent 第一次改文件之前会自动记一个。',
  'cp.diff': '差异',
  'cp.restore': '恢复…',
  'cp.restoreAnswer': '恢复到此回答之前',
  'cp.rollback': '回滚',
  'cp.restoreAnswerUnavailable': '此回答没有可恢复的文件改动',
  'cp.restoreAnswerTitle': '恢复此回答的改动？',
  'cp.restoreAnswerHint': '把工作区文件恢复到这条回答第一次修改文件之前的检查点；聊天记录保持不变。当前有 {n} 个路径与该检查点不同。',
  'cp.restoreAnswerNoop': '当前没有可恢复的工作区文件差异。Git 元数据（.git）会被刻意排除，不能在这里恢复。',
  'cp.restoreWorkspace': '恢复项目：{path}',
  'cp.baseline': '基线',
  'cp.noLabel': '（无标签）',
  'cp.restoreTitle': '恢复到检查点 {id}？',
  'cp.restoreNoop': '你的工作区已经和这个检查点一致，不会有任何变化。',
  'cp.restoreCount': '将写入或删除 {n} 个文件。你自己的 git 历史、索引和 stash 不会被触碰。',
  'cp.unprotected': '其中 {n} 个没有被你的 git 跟踪，也就是说这份工作副本是唯一的一份：',
  'cp.andMore': '…还有 {n} 个',
  'cp.restoring': '正在恢复…',
  'cp.forkFailed': '工作区文件已恢复，但把聊天切换到分支时失败：{message}',
  'cp.forkUnavailable': '工作区文件已恢复，但没能定位聊天分支的位置。重新打开这个会话即可继续。',
  'turn.filesChanged': '{n} 个文件已更改',
  'turn.checking': '正在检查文件改动…',
  'turn.toggleList': '展开或收起文件列表',
  'turn.review': '审查',
  'turn.open': '打开',
  'turn.edit': '编辑',
  'turn.copy': '复制',
  'turn.copied': '已复制',
  'turn.undo': '撤销',
  'turn.undoTitle': '撤销这一轮？',
  'turn.undoHint': '工作区文件将恢复到本轮 {n} 个文件改动之前；这一轮及之后的对话会从当前分支移除（原会话保留完整记录）。',
  'turn.editTitle': '编辑这一轮的提问',
  'turn.editHint': '发送后：文件先恢复到本轮改动之前，这一轮及之后的对话从当前分支移除，然后用修改后的提问重新回答。',
  'turn.sendEdit': '发送并重新回答',
  'turn.stillRunning': '本轮还在进行中',
  'turn.firstTurnNoFork': '会话的第一轮没有更早的分支点，无法撤销或编辑',
  'turn.firstTurnUndo': '这是会话的第一轮，没有更早的位置可以切断聊天。工作区文件会恢复，并将在同一项目新建一个会话。',
  'turn.trashFailed': '文件已恢复并新建了会话，但原会话未能归档进回收站（会话管理可能已关闭）。',
  'turn.editSendFailed': '分支已创建，但重新发送失败：{message}。可以在新会话里手动发送这条提问。',
  'cp.restored': '已恢复。恢复前先记了一个检查点，所以这一步可以撤销。',
  'cp.previewFailed': '算不出这次恢复会改动什么：{message}',
  'cp.emptyDiff': '（这个检查点没有文本变化）',
  'cp.checkpointN': '检查点 {id}',
  'cp.off': '检查点已关闭。',
  'cp.justNow': '刚刚',
  'cp.minsAgo': '{n} 分钟前',
  'cp.hoursAgo': '{n} 小时前',
  'cp.unknownTime': '时间未知',

  // Git tabs and sections
  'tab.git': 'Git 版本控制',
  'section.git': 'Git 版本控制与工作流',
  'section.git.desc': '代码提交与推送、AI 自动生成 Commit、分支管理以及会话绑定的独立 Worktree。',
  'git.section.commit': 'AI 提交信息生成策略',
  'git.section.binding': '会话与分支绑定机制',
  'git.section.worktree': 'Worktree 独立工作区',
  'git.section.push': '推送与远程协作安全策略',

  // Git settings options
  'git.model': '生成模型',
  'git.model.hint': '根据 git diff 生成提交信息的模型。留空时默认自动跟随当前会话的模型。',
  'git.model.followSession': '自动跟随当前会话模型（推荐）',
  'git.commitStyle': '提交信息格式规范',
  'git.commitStyle.conventional': 'Conventional Commits (feat/fix 行业标准规范)',
  'git.commitStyle.simple': '单行极简摘要',
  'git.commitStyle.detailed': '详细改动点列表',
  'git.commitLanguage': '生成语言偏好',
  'git.commitLanguage.zh': '简体中文 (zh-CN)',
  'git.commitLanguage.en': 'English (en)',
  'git.commitLanguage.auto': '跟随界面语言',
  'git.autoStageAll': '无暂存时自动暂存全部变更',
  'git.autoStageAll.hint': '当暂存区为空时，点击 AI 生成或点击提交自动执行全部暂存。',
  'git.sessionBinding': '会话绑定模式',
  'git.sessionBinding.strict': '严格模式：创建会话绑定分支且会话内强锁定 (推荐)',
  'git.sessionBinding.prompt': '提示模式：创建时提示绑定，允许手动解锁',
  'git.sessionBinding.off': '自由模式：不限制会话与分支',
  'balance.stale': '余额已过期',
  'sessions.gc.unsupported': '当前宿主未提供安全的附件删除接口，附件将保留。',
  'git.autoAlignBranch': '会话切换时自动对齐仓库分支',
  'git.autoAlignBranch.hint': '切换到某个会话时，自动将底层仓库分支对齐到该会话绑定的分支。',
  'git.worktreeDirPattern': '新建 Worktree 目录命名规则',
  'git.worktreeDirPattern.hint': '新建 Worktree 时的默认相对路径模板，支持 {repo} 和 {branch} 变量。',
  'git.worktreeAutoRegister': '自动注册为 DSH 工作空间',
  'git.worktreeAutoRegister.hint': '新建 Worktree 后自动将其添加到 DSH 侧边栏工作空间列表，支持独立多任务开发。',
  'git.pushAutoSetUpstream': '自动关联远程分支 (--set-upstream)',
  'git.pushAutoSetUpstream.hint': '推送本地新分支时，若尚未关联远程追踪分支，自动执行 --set-upstream。',
  'git.pushTimeout': '推送超时时间（秒）',
  'git.pushTimeout.hint': 'Git 推送操作的最长等待超时时间（秒）。',

  // Git review and explorer UI
  'git.commit': '提交',
  'git.commitAndPush': '提交并推送',
  'git.push': '推送',
  'git.pushCount': '推送 (↑{n})',
  'git.stageAll': '全部暂存',
  'git.unstageAll': '全部取消暂存',
  'git.stagedChanges': '暂存的更改',
  'git.unstagedChanges': '未暂存的更改',
  'git.changes': '更改',
  'git.unstageAllChanges': '取消暂存所有更改',
  'git.stageAllChanges': '暂存所有更改',
  'git.discardAllChanges': '放弃所有更改',
  'git.noStagedChanges': '无暂存的更改',
  'git.noUnstagedChanges': '无更改',
  'git.stageFile': '暂存此文件',
  'git.unstageFile': '取消暂存此文件',
  'git.discardChange': '放弃更改',
  'git.stageFolder': '暂存文件夹内全部文件',
  'git.unstageFolder': '取消暂存文件夹内全部文件',
  'git.discardFolder': '放弃文件夹内全部更改',
  'git.confirmDiscardAll': '确定要放弃全部 {n} 个文件的所有未暂存更改吗？',
  'git.confirmDiscardFile': '确定要放弃文件 "{file}" 的所有未暂存更改吗？',
  'git.confirmDiscardFolder': '确定要放弃文件夹 "{folder}" 下全部 {n} 个文件的未暂存更改吗？',
  'git.confirmDiscardAction': '放弃更改',
  'git.discardIrreversible': '此操作不可逆，工作区内这些未暂存的修改将永久丢失！',
  'git.aiGenerate': '✨ AI 生成',
  'git.aiGenerating': '正在生成...',
  'git.commitPlaceholder': '输入提交说明 (Commit Message) 或点击上方 ✨ AI 生成...',
  'git.stagedCount': '已暂存 {n} 项',
  'git.unstagedCount': '未暂存 {n} 项',
  'git.committed': '提交成功: {hash}',
  'git.pushed': '推送成功',
  'git.pushFailed': '推送失败: {message}',
  'git.commitFailed': '提交失败: {message}',
  'git.generateFailed': '生成提交说明失败: {message}',
  'git.branches': '分支管理',
  'git.worktrees': '工作区 (Worktree)',
  'git.searchBranches': '搜索分支...',
  'git.currentBranch': '当前分支',
  'git.lockedSession': '已与当前会话绑定锁定',
  'git.cannotSwitchInSession': '当前会话已锁定在分支 [{branch}]，不可在此切换分支。如需在其他分支工作，请开启新会话。',
  'git.openSession': '打开对应会话',
  'git.newSessionForBranch': '以此分支开启新会话',
  'git.createBranch': '新建分支',
  'git.createBranchBtn': '创建并检出',
  'git.branchName': '分支名称',
  'git.startPoint': '基准分支/提交（选填）',
  'git.createWorktree': '新建 Worktree',
  'git.worktreePath': '工作区目录路径',
  'git.useWorktree': '使用此工作区',
  'git.openInEditor': '在外部编辑器中打开',
  'git.removeWorktree': '移除工作区',
  'git.removeWorktreeConfirm': '确定移除工作区 {path} 吗？该目录将被物理删除。',
  'git.detached': '游离于 {sha}',
  'git.sessionPicker.title': '工作分支绑定',
  'git.sessionPicker.hint': '请确认本会话的工作分支，发送首条消息后将与本会话强锁定。',
  'git.sessionPicker.lockNotice': '已锁定分支: {branch}',
  'git.localBranches': '本地分支',
  'git.remoteBranches': '远程分支',
  'git.checkoutBranch': '检出分支',
  'git.checkoutSuccess': '已切换到分支 {branch}',
  'git.sessionLockedTooltip': '当前分支: {branch} (会话已锁定)',
  'git.worktreeMain': '主工作区',
  'git.worktreeDropdown': '工作区与 Worktree',
  'git.branchDropdown': 'Git 分支',
  'git.mainWorkspace': '主工作区 (默认)',
  'git.otherWorktrees': '关联 Worktrees',
  'git.noOtherWorktrees': '暂无其他 Worktree',
  'git.boundToWorktreeNotice': '当前处于关联 Worktree 中，分支已固定为【{branch}】。Git 规则规定 Worktree 独占其分支，如需自由切换分支，请选择主工作区。',
  'git.checkedOutInWorktree': '已在 Worktree 中检出',
  'git.createWorktreeAction': '创建 Worktree...',
  'git.createBranchAction': '创建分支...',
  'git.autoRegisterDsh': '自动注册为客户端工作区 (推荐)',
  'git.checkoutAfterCreate': '创建后立即切换到该分支',
  'git.useExistingBranch': '使用已有分支',
  'git.createNewBranch': '创建新分支',
  'git.worktreePrefix': 'Worktree:',
  'git.worktreeOfPrefix': '{name} 的 Worktree:',
  'git.worktreeTag': '{name} 的 Worktree',
  'git.mainWorkspacePrefix': '主工作区:',
  'git.branchPrefix': '分支:',
  'git.notRepository': '当前工作区还不是 Git 仓库',
  'git.initRepository': '初始化 Git 仓库',
  'git.initRepositoryHint': '在当前工作区执行 git init',
  'git.initSuccess': 'Git 仓库初始化成功',
  'git.initFailed': '初始化 Git 仓库失败: {message}',
}

/** Both dictionaries, in the shape `ctx.locale.register` takes. */
export const DICTS = { en, zh } as const
