...
async function askReviewer(ctx, settings, tool, command, callerSignal, session) {
  const llm = ctx.get("llm");
...
\`\`\``;
    ...
    const reusePrefix = header !== void 0 && headerConfig !== void 0 && headerConfig.provider != null && headerConfig.model != null && history !== void 0 && history.length > 0;
    ...
    return await cached(cacheKey, REVIEW_CACHE_TTL_MS, async () => {
      ...
      const stream = llm.stream({
        ...
        model,
        system,
        ...reusePrefix && header?.tools !== void 0 ? { tools: header.tools } : {},
        messages,
        maxTokens: 300,
        temperature: 0,
        ...reusePrefix && session?.id !== void 0 ? { sessionId: session.id } : {},
...
function mountCommandReview(ctx, config, routes, auditFile) {
  ...
  const dispose = ctx.on("tools/pre-execute", async function(exec2, next) {
    ...
    const verdict = await askReviewer(ctx, settings, exec2.name, command, exec2.signal, exec2.agent?.session);
    if (verdict === void 0) {
...
import { isAbsolute, join as join2, relative, resolve, sep, dirname as dirname2, basename, extname } from "node:path";
...
async function readTextFile(root, requested, scopeSuffix = "") {
  ...
  if (imageMime !== void 0) {
    ...
    const buffer2 = await readFile2(absolute);
  ...
  return await new Promise((resolve4, reject) => {
    try {
      if (process.platform === "win32") {
        ...
        child.unref();
        resolve4({ opened: true, editor: "file-explorer" });
      } else if (process.platform === "darwin") {
...
async function openInEditor(root, target, isFile, editorType) {
  ...
  return await new Promise((resolve4, reject) => {
    ...
    const watch = setTimeout(() => {
      settle(() => {
        child.unref();
        resolve4({ opened: true, editor: launcher ?? "" });
      });
    }, LAUNCH_WATCH_MS);
    child.on("error", (error) => {
      settle(() => {
    ...
    child.on("close", (code) => {
      settle(() => {
        if (code === 0) resolve4({ opened: true, editor: launcher ?? "" });
...
import { basename as basename2, isAbsolute as isAbsolute2, resolve as resolve2 } from "node:path";
...
function mountGitOps(ctx, config, routes, bindingFilePath) {
  ...
  const handleWorktreeAdd = async ({ body, req }) => {
    ...
    const absoluteTarget = isAbsolute2(targetPath) ? resolve2(targetPath) : resolve2(repo, targetPath);
      ...
      if (untrackedPaths.length > 0) {
        ...
        if (!res.ok) {
          ...
          for (const up of untrackedPaths) {
            await rm2(resolve2(repo, up), { recursive: true, force: true }).catch(() => {
            });
          }
        }
      }
...
import { rm as rm3, stat as stat2 } from "node:fs/promises";
    ...
    "/sessions/purge": async ({ body, method, req }) => {
            ...
            try {
              ...
              if (table) {
                await table.update(entity.id, (record) => ({
                  ...record,
                  sessionIds: record.sessionIds.filter((id) => String(id) !== target),
      ...
      try {
        ...
        for (const target of targets) {
          ...
          if (match !== void 0) {
            try {
              await rm3(match.path, { force: true, recursive: true });
              const parentDir = dirname3(match.path);
              if (basename3(parentDir) === target) {
                await rm3(parentDir, { force: true, recursive: true });
              }
            } catch (e) {
...
import { dirname as dirname4, isAbsolute as isAbsolute3, join as join4, resolve as resolve3 } from "node:path";
...
async function bundleRowIds(packageJsonPath) {
  ...
  const patchPath = isAbsolute3(pointer) ? pointer : resolve3(dirname4(packageJsonPath), pointer);
  try {
...
import { mkdir as mkdir2, readFile as readFile7, rm as rm4, writeFile } from "node:fs/promises";
...
var CheckpointStore = class {
  ...
  async clearStaleLock(repo) {
    await rm4(`${join6(repo.gitDir, "dsh-index")}.lock`, { force: true }).catch(() => {
    });
  }
...
${label}`;
  ...
  async linkAnchor(workTree, sessionId, checkpointId, anchorSeq) {
    const repo = this.repoFor(workTree);
    const file = join6(repo.gitDir, "dsh-turn-anchors", `${workspaceKey(sessionId)}.json`);
    await mkdir2(dirname5(file), { recursive: true, mode: 448 });
    let index = {};
    try {
      index = JSON.parse(await readFile7(file, "utf8"));
    } catch {
    }
...
`, { mode: 384 });
  ...
  async resolveAnchor(workTree, sessionId, checkpointId) {
    ...
    try {
      const file = join6(repo.gitDir, "dsh-turn-anchors", `${workspaceKey(sessionId)}.json`);
      const index = JSON.parse(await readFile7(file, "utf8"));
  ...
  async turnChanges(workTree, refs, endRefs, turn, signal) {
    ...
    if (to === void 0) {
      const tempIndex = join6(repo.gitDir, `dsh-turn-index-${process.pid}`);
      try {
        ...
        await rm4(`${tempIndex}.lock`, { force: true }).catch(() => {
        });
        await rm4(tempIndex, { force: true }).catch(() => {
        });
      }
    }
  ...
  async restore(workTree, sessionId, checkpointId, signal) {
    ...
    for (const path of toRemove) {
      try {
        await rm4(join6(workTree, path), { force: true });
        removed += 1;
      } catch {
      }
    }
  ...
  async forget(workTree) {
    await rm4(this.repoFor(workTree).gitDir, { recursive: true, force: true });
  }
...
function mountCheckpoints(ctx, config, routes, checkpointRoot) {
  ...
  const store2 = new CheckpointStore(checkpointRoot, settings.excludes, settings.maxFileSizeMb);
...
`)) {
  ...
  const contributed = installRoutes(routes, {
    ...
    "/checkpoints/turn-info": async ({ query, req }) => {
      ...
      if (cached2 !== void 0 && cached2.refsHash === currentRefsHash && now - cached2.timestamp < TURN_CHANGES_TTL) {
        ...
        changes = await store2.turnChanges(workTree, refs, endRefs, resolvedTurn, controller.signal);
    ...
    "/checkpoints/preview": async ({ query, req }) => {
      ...
      const { affected, unprotected } = await store2.preview(workTree, id, controller.signal);
    ...
    "/checkpoints/restore": async ({ body, method, query, req }) => {
      ...
      const workTree = await requireWorkspace(ctx, query.get("workspace"), sessionId, controller.signal);
      try {
        const result = await serialize(workTree, () => store2.restore(workTree, sessionId, request.id, controller.signal));
    ...
    "/checkpoints/prune": async ({ method, query }) => {
      ...
      return { pruned: await store2.prune(workTree, current.retentionDays) };
    },
  ...
  if (retention > 0) {
    ...
    if (workTree !== void 0) {
      void store2.prune(workTree, retention).catch((error) => {
