...
function resolveRoot(ctx, requestedId, sessionId) {
...
import { mkdir as mkdir2, readFile as readFile2, readdir as readdir2, rename, rm as rm2, stat as stat2 } from "node:fs/promises";
...
async function readTrash(trashRoot) {
  ...
  for (const id of entries) {
    try {
      const manifest = JSON.parse(await readFile2(join3(trashRoot, id, MANIFEST_NAME), "utf8"));
      let sizeBytes = 0;
      try {
  ...
  return installRoutes(routes, {
    ...
    "/sessions/delete": async ({ body, method, req }) => {
      ...
      try {
        ...
        if (error.code === "EXDEV") {
          await writeFileAtomic2(join3(entryDir, artifactName), await readFile2(located.path, "utf8"), { mode: 384, dirMode: 448 });
          await rm2(located.path, { force: true });
        } else {
    ...
    "/sessions/restore": async ({ body, method }) => {
      ...
      let manifest;
      try {
        manifest = JSON.parse(await readFile2(join3(entryDir, MANIFEST_NAME), "utf8"));
      } catch {
      ...
      try {
        ...
        if (error.code === "EXDEV") {
          await writeFileAtomic2(manifest.originalPath, await readFile2(source, "utf8"), { mode: 384 });
...
async function readProfile(dir) {
  try {
    const parsed = JSON.parse(await readFile5(join5(dir, "package.json"), "utf8"));
...
import { mkdir as mkdir3, readFile as readFile6, rm as rm3, writeFile } from "node:fs/promises";
...
${label}`;
  ...
  async exists(workTree) {
    try {
      await readFile6(join6(this.repoFor(workTree).gitDir, "HEAD"), "utf8");
      return true;
    } catch {
      return false;
    }
