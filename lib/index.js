...
function isReadOnlyCall(ctx, exec, command, patterns) {
  try {
    const definition = ctx.get("tools")?.get(exec.name, exec.agent);
...
function turnForMessageEvents(events, messageId) {
  for (let index = events.length - 1; index >= 0; index -= 1) {
    const event = events[index];
...
function requireWorkspace(ctx, requested, sessionId) {
...
function mountCheckpoints(ctx, config, routes, checkpointRoot) {
  ...
  async function turnForMessage(sessionId, messageId, signal) {
    const persistence = ctx.get("sessionPersistence");
    if (persistence === void 0) return void 0;
  ...
  const disposeHook = ctx.on("tools/pre-execute", async function(exec, next) {
    ...
    try {
      await serialize(workTree, async () => {
        ...
        if (turn !== void 0) await store.linkTurn(workTree, sessionId, turn, snapshot.id);
      });
    } catch (error) {
  ...
  const contributed = installRoutes(routes, {
    ...
    "/checkpoints/for-message": async ({ query, req }) => {
      ...
      if (turn === void 0) return { checkpoint: null };
    ...
    "/checkpoints/preview": async ({ query, req }) => {
      ...
      const workTree = requireWorkspace(ctx, query.get("workspace"), query.get("session") ?? void 0);
      const controller = new AbortController();
      req.on("close", () => {
        controller.abort();
      });
