...
function mountCommandReview(ctx, config, routes, auditFile) {
  ...
  const dispose = ctx.on("tools/pre-execute", async function(exec2, next) {
    ...
    if (matched === void 0 && settings.mode !== "all" && !COMMAND_MUTATION_PATTERN.test(command)) return await next();
  ...
  return await new Promise((resolve5, reject) => {
    try {
      if (process.platform === "win32") {
        ...
        child.unref();
        resolve5({ opened: true, editor: "file-explorer" });
      } else if (process.platform === "darwin") {
...
async function openInEditor(root, target, isFile, editorType) {
  ...
  return await new Promise((resolve5, reject) => {
    ...
    const watch = setTimeout(() => {
      settle(() => {
        child.unref();
        resolve5({ opened: true, editor: launcher ?? "" });
      });
    }, LAUNCH_WATCH_MS);
    child.on("error", (error) => {
      settle(() => {
    ...
    child.on("close", (code) => {
      settle(() => {
        if (code === 0) resolve5({ opened: true, editor: launcher ?? "" });
...
import { lstat as lstat2, rm as rm2, stat as stat2 } from "node:fs/promises";
    ...
    "/sessions/purge": async ({ body, method }) => serialize(async () => {
      ...
      for (const target of targets) {
        try {
          ...
          if (match !== void 0) {
            if (!isAbsolute3(match.path)) throw new Error("persistence returned a non-absolute artifact path");
            const info = await lstat2(match.path).catch((error) => {
...
async function bundleRowIds(packageJsonPath) {
  ...
  const patchPath = isAbsolute4(pointer) ? pointer : resolve3(dirname3(packageJsonPath), pointer);
  try {
...
async function updateQuarantine(recordFile, patchFile, mutate) {
  await mkdir2(dirname4(recordFile), { recursive: true, mode: 448 });
...
import { lstat as lstat3, mkdir as mkdir3, readFile as readFile7, readdir as readdir3, rm as rm3, writeFile } from "node:fs/promises";
  ...
  async stage(repo, signal, indexFile) {
    ...
    for (let at = 0; at < candidates.length; at += 64) {
      ...
      await Promise.all(candidates.slice(at, at + 64).map(async (path) => {
        ...
        try {
          const info = await lstat3(join6(repo.workTree, path));
...
${label}`;
  ...
  async linkAnchor(workTree, sessionId, checkpointId, anchorSeq) {
    ...
    await mkdir3(dirname5(file), { recursive: true, mode: 448 });
    let index = {};
    try {
...
`, { mode: 384 });
  ...
  async restore(workTree, sessionId, checkpointId, signal) {
    ...
    for (const path of targetPaths) {
      ...
      const info = await lstat3(join6(workTree, path)).catch((error) => {
        if (error.code === "ENOENT") return void 0;
        throw error;
      });
...
` });
    ...
    const anchorsDir = join6(repo.gitDir, "dsh-turn-anchors");
    ...
    for (const name2 of anchorFiles.filter((name3) => name3.endsWith(".json"))) {
      const file = join6(anchorsDir, name2);
      const index = JSON.parse(await readFile7(file, "utf8"));
      for (const id of expired) delete index[id];
      await writeFileAtomic4(file, `${JSON.stringify(index)}
`, { mode: 384 });
    }
    await writeFileAtomic4(join6(repo.gitDir, "shallow"), `${oldest.id}
`, { mode: 384 });
...
`)) {
  ...
  function serialize(workTree, operation) {
    const absolute = resolve4(workTree);
    ...
    "/checkpoints/restore": async ({ body, method, query, req }) => {
      ...
      try {
        const result = await serialize(workTree, () => {
          const normalize = (path) => process.platform === "win32" ? resolve4(path).toLowerCase() : resolve4(path);
